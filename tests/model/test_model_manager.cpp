...
protected:
  ...
  {
    Model_Manager manager;
    auto& var = manager.var(manager.make_var("x", false));
    manager.set_var_lower_bound(var, 0.5);
    manager.set_var_upper_bound(var, 2.5);
    manager.set_var_type(var, Var_Type::general_integer);
    ...
    manager.set_var_type(var, Var_Type::fixed);
    check(var.requires_integrality(),
          "Fixing an integer variable should preserve integrality");

    double near_integer =
        1.0 + 0.5 * k_default_feas_tolerance;
    check(manager.normalize_var_value(var, near_integer),
    ...
    check(!manager.normalize_var_value(var, fractional),
          "Clearly fractional integer values should be rejected");

    double out_of_bounds =
        3.0 + 0.5 * k_default_feas_tolerance;
    const double original_value = out_of_bounds;
    check(!manager.normalize_var_value(var, out_of_bounds),
    ...
    check(!is_integral_within_tolerance(
              0.5, k_default_feas_tolerance),
          "Fractional values should not pass integral-value checks");
    check(is_integral_within_tolerance(
              1.0 + 0.5 * k_default_feas_tolerance,
              k_default_feas_tolerance),
          "Tolerance-level integer noise should be recognized");

    auto& binary = manager.var(manager.make_var("b", false));
    manager.set_var_lower_bound(binary, 2.0);
    manager.set_var_upper_bound(binary, 3.0);
    manager.set_var_type(binary, Var_Type::binary);
    ...
    invalid_manager.make_con("");
    auto& invalid_bounds =
        invalid_manager.var(invalid_manager.make_var("nan", false));
    invalid_manager.set_var_lower_bound(
        invalid_bounds, std::numeric_limits<double>::quiet_NaN());
    check(!invalid_manager.process_after_read(),
    ...
    tolerant_manager.set_var_lower_bound(tolerant_var, 1.0);
    tolerant_manager.set_var_upper_bound(
        tolerant_var, 1.0 - 0.5 * k_default_feas_tolerance);
    ...
    check(tolerant_manager.normalize_var_value(tolerant_var, fixed_value),

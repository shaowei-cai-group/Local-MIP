...
std::vector<double> generate_start(Model_Manager& p_manager,
...
bool test_objective_guided_minimize()
{
  ...
  const int epsilon_positive = api.add_var(
      "epsilon_positive", -6.0, 6.0, k_zero_tolerance);
  const int epsilon_negative = api.add_var(
      "epsilon_negative", -6.0, 6.0, -k_zero_tolerance);
  const int above_epsilon = api.add_var(
      "above_epsilon", -6.0, 6.0, 2.0 * k_zero_tolerance);

  Model_Manager manager;
  manager.set_bound_strengthen(0);
  api.build_model(manager);
  bool ok = check(manager.process_after_read(),
                  "Minimization model should preprocess successfully");
...
{
  Model_API api;
  api.set_sense(Model_API::Sense::maximize);
  ...
  api.build_model(manager);
  bool ok = check(manager.process_after_read(),
                  "Maximization model should preprocess successfully");
...
bool test_lock_guided()
{
  ...
  Model_Manager manager;
  manager.set_bound_strengthen(0);
  manager.set_split_eq(false);
  api.build_model(manager);
  bool ok = check(manager.process_after_read(),
                  "Lock-guided model should preprocess successfully");
  ok &= check(manager.con(4).is_equality(),
              "Equality should remain unsplit for the lock test");
  manager.con(5).mark_inferred_sat();
...
bool test_split_equality_locks()
{
  ...
  api.build_model(manager);
  bool ok = check(manager.process_after_read(),
                  "Split-equality model should preprocess successfully");
